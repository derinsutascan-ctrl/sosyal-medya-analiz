...
# --- 3. OTURUM KONTROLÜ (Girişi Hatırlayan Bölüm) ---
...
if not st.session_state.oturum_durumu:
    ...
    # --- 5. ANA PANEL (Orijinal Tasarımın) ---
    df = veri_yukle()
    with st.sidebar:
        ...
        # ADMIN PANELİ (Ekip Arkadaşı Ekleme)
        ...
        if sayfa_modu == "📊 Marka Bazlı Detay":
            ...
            secilen_ay = st.selectbox("Ay Seçin:", df['Ay'].unique() if not df.empty else ["Veri Yok"])
    ...
    # --- 6. RAPORLAMA VE GRAFİKLER (SENİN ORİJİNAL KODLARIN) ---
    if sayfa_modu == "🏠 Genel Bakış":
        ...
        # Senin Marka Bazlı Detay Grafik Kodların
